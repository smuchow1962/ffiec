...
**Optional carve-outs (forward-pointer).** Eight §10.x sections define OPTIONAL extensions institutions opt into when their deployment context requires them; institutions that decline any of these remain v1.x-conformant under the primary scope above. The eight are: §10.31 per-cohort Merkle subtree disclosure (multi-jurisdiction or multi-tenant SaaS institutions disclosing a cohort-bounded subtree to one regulator without disclosing leaves outside that regulator's authority); §10.32 per-device session-key derivation (federated-learning workers, edge-AI tablets, point-of-sale terminals binding the chain to a device identity in addition to the tenant); §10.33 model-update events (federated-learning and edge-AI deployments capturing the deployment-phase boundary on the chain); §10.34 `audit.training.*` event family (institutions that want a single chain-anchored evidence trail covering training-phase lifecycle documentation under AI Basic Act 2025 Article 20-2 and EU AI Act Article 12); §10.35 edge-attestation envelope (chain entries bound to hardware-rooted device attestation under Android Keystore, Apple Secure Enclave, TPM, or Google Pixel StrongBox); §10.36 late-arriving-entry seal discipline (Pattern A supplemental or Pattern B rolling-window for events that arrive after their seal day is published); §10.37 hierarchical Merkle aggregation (two-level tree with per-subtree §10.31 roots and a top-level outer tree, for bandwidth-efficient cohort disclosures at scale); §10.38 `audit.consent.*` event family (institutions operating under privacy regimes that mandate explicit consent records — DPDP Act, GDPR, CCPA/CPRA, PIPA, APPI).
...
Edge-AI deployments (federated-learning workers, mobile tablets in field-office workflows, point-of-sale terminals, TPM-backed workstations) bind chain entries to a hardware-rooted attestation of the device's identity. §10.35 normates this as the `ffiec.chain.attestation` attribute on chain entries plus a delegate-dispatch validator pattern that platform-specific attestation parsers plug into at chain-walk time.

**Attestation envelope schema (normative when applicable).** The `ffiec.chain.attestation` attribute is a structured object with the following fields:
...
**Platform enumeration (normative starting set; future amendments may extend).** Values are byte-locked across implementations:

- `"android_keystore"` — Android Keystore Hardware-Backed Key Attestation (CBOR / X.509 cert chain to Google's Hardware-Backed Keys root)
- `"apple_secure_enclave"` — Apple DeviceCheck / Secure Enclave attestation (Apple's PKCS#7 attestation tied to Apple's root CA)
- `"tpm"` — TPM 2.0 attestation (TCG Platform Attestation; vendor-specific intermediate roots)
- `"google_pixel_strongbox"` — Google Pixel StrongBox-backed attestation (a hardened Android Keystore variant with separate root)

**Delegate-dispatch validator (normative).** §10.35-conformant verifiers hold a registry mapping platform → platform-specific attestation validator. Each platform validator is responsible for parsing the platform-specific attestation document, walking the certificate / signature chain to the named root, and confirming the device's public key (when `public_key_fingerprint` is present, the validator MUST cross-check it against the attested key). Platform validators MUST return a boolean (success / failure) rather than throwing on invalid attestations — a thrown exception escapes the dispatcher's anomaly-recording surface.

The verifier dispatcher reads `envelope["platform"]`, looks up the registered validator, and invokes it with `(attestation_document_b64, root_certificate_id)`. An envelope with a platform not in the §10.35 enumeration, or a platform with no registered validator, is reported as a control-completeness failure (the institution's CC8.1 names the supported platforms; an envelope outside that set indicates a configuration drift).

**Composition with §10.32 (per-device key derivation).** §10.32-bound chains carry `device_id` in the HKDF info; §10.35 binds the device's hardware-rooted identity to a chain entry. The two compose: an institution emitting both stamps the device's `device_id` in its session key AND attaches the device's hardware attestation. The verifier cross-checks: the `device_id` HKDF binding is what ties events to the device; the §10.35 envelope is what ties the device to physical hardware via the platform-vendor's attestation chain.

**Cross-reference.** §1 scope (decision-time integrity is the primary scope; §10.35 is an OPTIONAL extension); §10.32 per-device session-key derivation; §10.29 streaming verifier (the delegate-dispatch validator pattern §10.35 mirrors); test-vector candidate `025-attestation-android-keystore` covers the Android Keystore platform end-to-end with a synthetic attestation document.
...
§4.2's daily seal Merkle root is computed RFC 6962-faithfully over the day's leaves in flat order. For institutions with many devices and many events per device (Saraswati Microfinance: 15,000 tablets × ~1,000 events/day) the flat-tree audit-path bandwidth for a cohort disclosure (one device's worth of events) is dominated by per-leaf overhead. §10.37 normates a two-level tree that lets a cohort disclosure share a single subtree root across all the leaves in that subtree.

**Two-level tree construction (normative).**

- **Level 1 — per-subtree tree.** Each subtree (typically per device under §10.32, but the boundary is institution-defined) is its own §10.31 RFC 6962 tree. The subtree's root is computed via `compute_merkle_root(leaves)` — RFC 6962 leaf hashes (`SHA-256(0x00 || leaf)`) plus internal hashes (`SHA-256(0x01 || L || R)`) per §10.31.
- **Level 2 — top tree.** The top-level tree is constructed over the subtree roots **directly** — no `LEAF_HASH` prefix at this level because subtree roots are already SHA-256 outputs (treating them as raw leaves and applying `LEAF_HASH` would add an unnecessary domain-separation layer). Internal nodes use the same `INTERNAL_HASH(0x01 || L || R)` primitive as §10.31.

```
Top root
└── INTERNAL_HASH(L, R)                              ← top-level combine
    ├── L = INTERNAL_HASH(rA, rB)                    ← outer internal node (no LEAF_HASH)
    │   ├── rA = MerkleRoot([LEAF_HASH(a0), ...])    ← per-subtree §10.31 root (RFC 6962)
    │   │   ├── LEAF_HASH(a0)                        ← inner-subtree leaves
    │   │   └── LEAF_HASH(a1), ...
    │   └── rB = MerkleRoot([LEAF_HASH(b0), ...])
    └── R = INTERNAL_HASH(rC, rD)                    ← outer internal node (no LEAF_HASH)
        ├── rC = MerkleRoot([LEAF_HASH(c0), ...])
        └── rD = MerkleRoot([LEAF_HASH(d0), ...])
```

The diagram shows a 4-subtree top tree. The inner level (per subtree) follows RFC 6962 with `LEAF_HASH` on each leaf. The outer level combines already-hashed subtree roots directly via `INTERNAL_HASH`, with no further `LEAF_HASH` wrapping.

The top-tree-root recursion follows RFC 6962's split-at-largest-power-of-2-strictly-less-than-n discipline:

- 0 subtrees → `SHA-256(b"")` (matches §4.2's empty-day pin).
- 1 subtree → the lone subtree's root (no `LEAF_HASH` wrap; the subtree root IS the top root).
- 2+ subtrees → split at `k = largest_power_of_2_less_than(n)`; recursively combine via `INTERNAL_HASH(left_top_root, right_top_root)`.

**Hierarchical inclusion proof (normative).** An audit path for one leaf is the **concatenation** of (a) the inner path within the target subtree (per §10.31 audit-path encoding) and (b) the outer path between subtree roots (§10.37's no-`LEAF_HASH` encoding, otherwise identical to §10.31). Both paths use the same `(sibling_hash, is_left)` step shape and combine via `INTERNAL_HASH`, so §10.31's `verify_audit_path` verifies the concatenated path unchanged.

Bandwidth: for a cohort disclosure of N leaves all in the same subtree of size K (in a top tree of M subtrees), the disclosure carries N inner paths (each `log2(K)` deep) plus one shared outer path (`log2(M)` deep) — vs the flat-tree approach which would emit N paths each `log2(N×K)` deep. Saraswati's 15,000-device × 1,000-events workload: flat path is ~24 hashes per leaf; hierarchical path within a single device is `log2(1000) + log2(15000)` ≈ 10 + 14 = 24 hashes per leaf, BUT the 14-deep outer path is shared across all 1000 leaves of that device — so the disclosure size is 1000×10 + 14 + 1 ≈ 10015 hashes vs flat 1000×24 = 24000 hashes (~58% bandwidth saving).

**Optionality.** §10.37 is OPTIONAL. Institutions whose daily leaf count is small enough that flat-tree audit paths are not bandwidth-bound do not need it. The §4.2 daily seal record's Merkle root is unchanged; institutions that opt into §10.37 publish their hierarchical root in addition to or instead of the flat root, naming the choice in CC8.1.

**Subtree-boundary integrity (normative when applicable).** The institution's CC8.1 names the boundary that defines subtrees (typically `device_id` per §10.32, but institutions may choose other boundaries — `tenant_id` for multi-tenant SaaS, `region` for multi-region resilience under §10.15) and the integrity mechanism guaranteeing the subtree-id-to-leaves mapping is not attacker-controlled. The §10.37 outer tree treats subtree roots as already-hashed values; second-preimage resistance of the inner tree (RFC 6962 §2.1 over SHA-256) holds, but boundary integrity is institution-side. An attacker controlling subtree-boundary assignment can shuffle leaves between subtrees, producing different hierarchical roots for the same flat leaf set — the chain entries themselves are unchanged but the per-subtree disclosure becomes an attacker-controllable view. CC8.1 describes the binding that prevents this (e.g., the device-attestation-bound `device_id` per §10.35).

**Equivalence with flat tree.** The hierarchical and flat §4.2 roots coincide when the subtree boundaries fall on the flat tree's natural split points (each subtree size is a power of 2 AND the cumulative leaf counts align with the flat split-at-largest-power-of-2 boundaries). For arbitrary subtree sizes, the two roots differ. Institutions opting into §10.37 publish their hierarchical root distinctly so verifiers do not confuse the two.

**Cross-reference.** §4.2 daily seal Merkle root (the flat-tree default); §10.31 per-cohort Merkle subtree disclosure (the inner-tree audit-path encoding §10.37 reuses); RFC 6962 §2.1 / §2.1.1 (the inner-tree leaf and internal hash primitives); §10.32 per-device session-key derivation (the natural subtree boundary for federated-learning / edge-AI deployments); test-vector candidate `026-hierarchical-merkle-aggregation` covers a 4-subtree top-tree with audit-path round-trip for every leaf.
...
| v1.0b edge-attestation + hierarchical Merkle extensions | 2026-05-08 | **§10.35 edge-attestation primitive + §10.37 hierarchical Merkle aggregation (both normative when applicable).** Pair B of the §10.35-§10.38 wishlist closure. **§10.35** adds the `ffiec.chain.attestation` attribute on chain entries plus a delegate-dispatch validator pattern. Platform enumeration: `android_keystore`, `apple_secure_enclave`, `tpm`, `google_pixel_strongbox`. Envelope schema: `platform` + `attestation_document_b64` (base64-encoded, ≤ 64 KiB, strict RFC 4648 §4 standard alphabet with no whitespace) + `root_certificate_id` (≤ 256 chars) + optional `public_key_fingerprint` (64 lowercase hex). Platform-specific parsers plug in via `AttestationValidator` delegates registered with the dispatcher; the dispatcher routes envelopes to the right validator at chain-walk time (mirrors §10.29 StreamingVerifier delegate pattern). Composes with §10.32 per-device key derivation: the `device_id` HKDF binding ties events to the device; the §10.35 envelope ties the device to hardware via the platform-vendor's attestation chain. **§10.37** adds a two-level Merkle aggregation: per-subtree §10.31 RFC 6962 roots at level 1, top-level outer tree using INTERNAL_HASH directly (no LEAF_HASH at the inter-subtree level because subtree roots are already SHA-256 outputs). `verify_audit_path` from §10.31 verifies the concatenated inner+outer audit path unchanged. Empty-tree pin matches §4.2's `e3b0c44...b855`. Cross-language pin: 4-subtree aggregation produces top root `3e3c7ee2...c704` byte-identical Python/.NET. Bandwidth saving for Saraswati's 15K-device × 1K-event workload: ~58% reduction vs flat-tree disclosure (10015 hashes vs 24000). §1 scope-boundary forward-pointer extended from six to eight optional carve-outs. Subtree-boundary integrity is institution-side (CC8.1 names the boundary mechanism — typically the §10.35 device-attestation-bound `device_id`). **Wire-form preservation.** Neither §10.35 nor §10.37 changes the `sign_payload` byte form. §10.35 is an attribute on chain entries under the existing v1.0b form; §10.37 produces an alternative root the institution may publish in addition to or instead of the flat §4.2 root, with the choice named in CC8.1. **Cross-references.** §10.35 → §10.32 / §10.29 / §1; §10.37 → §4.2 / §10.31 / RFC 6962 / §10.32. |
